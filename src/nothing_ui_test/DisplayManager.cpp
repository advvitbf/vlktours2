...
#include "../../final_eye_pack_nothing_style/mochi_nothing_style_eye_pack_split.h"
#endif
...
void DisplayManager::playEyesSequence(bool restart, uint8_t startAnimation, uint64_t mask)
{
    ...
    const MochiNothingStyleEyeAnimation &animation = MOCHI_NOTHING_STYLE_EYE_ANIMATIONS[_eyesSequenceAnimation];
...
#if CUBIE_USE_OLED
    const MochiNothingStyleEyeAnimation &animation = MOCHI_NOTHING_STYLE_EYE_ANIMATIONS[_eyesSequenceAnimation];
    _display.drawBitmap(0, 0, animation.frames[_eyesSequenceFrame], MOCHI_NOTHING_STYLE_EYE_WIDTH, MOCHI_NOTHING_STYLE_EYE_HEIGHT, SH110X_WHITE);
    if (!_eyesSequencePlaying)
    {
        return;
    }
...
void DisplayManager::renderMenu(const DisplayFrame &frame)
{
    ...
    {
        drawListItem(i, labels[i], i == frame.menuIndex);
    }
}
...
void DisplayManager::renderApps(const DisplayFrame &frame)
{
    ...
    {
        drawListItem(i, labels[i], i == frame.appsIndex);
    }
}
...
void DisplayManager::renderStopwatch(const DisplayFrame &frame)
{
    ...
    _display.print(frame.stopwatch.running ? "Counting" : "Idle");
    ...
    _display.setCursor((CubieConfig::OledWidth - w) / 2, 36);
...
void DisplayManager::renderNavigation(const DisplayFrame &frame)
{
    ...
    drawUiChrome("Move");
    printDefault(navigationLabel(frame.navigation.icon), 8, 12);
...
{
    drawUiChrome("State");
    drawMetricRow(16, "Ble", frame.bleConnected ? "On" : "Off");
    drawMetricRow(30, "Phone", String(frame.battery.phonePercent) + "%");
    drawMetricRow(44, "Cubie", String(frame.battery.cubiePercent) + "%");
}

void DisplayManager::renderBatteryAlert(const DisplayFrame &frame)
{
    drawUiChrome("Power");
    centerText("Low energy", 16);
    centerText(String(frame.battery.cubiePercent) + "%", 28);
    centerText("Plug me in", 52);
}
...
{
    drawUiChrome("Ble");
    centerText(strcmp(state, "CONNECTED") == 0 ? "Linked" : "Lost", 28, 2);
}
...
void DisplayManager::renderSettings(const DisplayFrame &frame)
{
    const char *labels[] = {
        ...
        frame.buzzerEnabled ? "Sound on" : "Sound off"};

    drawUiChrome("Tune");

    for (uint8_t i = 0; i < 3; i++)
    {
        drawListItem(i, labels[i], i == frame.settingsIndex);
    }
}

void DisplayManager::drawUiChrome(const char *label)
{
    _display.setFont();
    _display.setTextColor(SH110X_WHITE);
    _display.setTextSize(1);
    _display.drawPixel(0, 0, SH110X_WHITE);
    _display.drawPixel(1, 0, SH110X_WHITE);
    _display.drawPixel(0, 1, SH110X_WHITE);
    _display.drawPixel(126, 0, SH110X_WHITE);
    _display.drawPixel(127, 0, SH110X_WHITE);
    _display.drawPixel(127, 1, SH110X_WHITE);
    _display.setCursor(6, 1);
    _display.print(label);
    for (int16_t x = 6; x < 34; x += 3)
    {
        _display.drawPixel(x, 10, SH110X_WHITE);
    }
}

void DisplayManager::drawListItem(uint8_t index, const char *label, bool selected)
{
    int16_t y = 16 + (index * 10);
    if (selected)
    {
        _display.fillRect(4, y - 2, 3, 7, SH110X_WHITE);
    }
    else
    {
        _display.drawPixel(6, y + 2, SH110X_WHITE);
    }

    _display.setCursor(14, y);
    _display.print(label);
}

void DisplayManager::drawMetricRow(int16_t y, const char *label, const String &value)
{
    _display.setTextSize(1);
    _display.setCursor(8, y);
    _display.print(label);
    _display.setCursor(88, y);
